...
- **Versão atual:** `2.6.2u` (em `backend/main.py` → `app.version`)
...
### 2026-07-15 (sessão 24 - Antigravity)
**O que foi feito:** Reformulação da hierarquia e usabilidade do editor de templates (`TemplatesRef.jsx`) no frontend e correção do comportamento de erros de referências do motor de fórmulas.
- **Hierarquia Visual e Grid Control:** Removidos inputs manuais de Modo de Cálculo, Nível e Ordem dos formulários de edição e inserção. Adicionados botões de movimentação na grade (⬆️/⬇️ para reordenar e ⬅️/➡️ para recuo visual/outdent/indent).
- **Cálculo Implícito:** Modo de cálculo resolvido automaticamente a partir do nível da linha e da presença da checkbox "Conta de Resultado (Fórmula)".
- **Visualizador de Filhas:** Adicionado indicador visual `"Soma de: X filhas"` com modal colapsável para inspecionar os nomes das linhas filhas associadas.
- **Busca de Variáveis:** Adicionada barra de filtragem rápida de variáveis no editor e configurados os botões de variáveis para inserir apenas o nome limpo no editor (sem chaves `{}`).
- **Correção no Backend (ref_formula_engine.py):** Ajustada a checagem no motor de fórmulas para lançar erro `NameError` caso as variáveis marcadas em chaves legadas `{agrupamento:xyz}` ou `{linha:abc}` não existam, garantindo que o erro `ref_inexistente` seja propagado nos testes em vez de ser silenciado com `0.0`.
- **Validação Local:** Compilado o frontend (`npm run build`) e executados os testes. Toda a suíte de **111 testes passou com sucesso (100% verde)**.
**Decisões tomadas:** As modificações de usabilidade e visualização da grade de templates estão implementadas localmente, mas a DRE estruturada **não está concluída** (faltando a UI de de-para conta-linha ativa e a importação real).
**Próximo passo:** Retomar a UI de de-para direto conta→linha da DRE e a importação de dados.

### 2026-07-15 (sessão 23)
**O que foi feito:** Conclusão e validação da **Camada 1** (esqueleto DRE real com 400 linhas via `seed_dre_controladoria.py`) e **Camada 1.5** (parâmetros de cliente para rateio de ADM por faturamento/percentual e presunção de perdas).
- **Testes automatizados:** Suíte de testes atualizada, totalizando **111 testes 100% verdes** (incluindo cobertura para rateio administrativo por faturamento, rateio por percentual fixo, regra de perdas presumidas e comportamento de fórmulas).
- **Frontend atualizado:** Redimensionado o modal do editor de template (`TemplatesRef.jsx`) com largura/altura de 99vw/96vh para melhor usabilidade na visualização da DRE e executado `npm run build` para manter o `frontend/dist/` sincronizado com o código fonte, passando com sucesso no teste de integridade local.
- **Banco de Dados:** Atualizadas as tabelas `clientes` (adicionando `percentual_perdas_presumido` e `criterio_rateio_adm`) e `unidades` (adicionando `eh_adm` e `percentual_rateio`) com migrações automáticas de inicialização no `backend/main.py`.
- **Carga de Dados:** O script `seed_dre_controladoria.py` foi atualizado e executado com sucesso no SQLite local, populando 400 linhas do template "Controladoria - DRE Varejo" e associando-o ao cliente Leal.
**Decisões tomadas:** Os testes e a carga de dados local validam o correto funcionamento das Camadas 1 e 1.5. Próximo passo é consolidar as alterações na branch `feature/dre-camada1-esqueleto-real` e seguir para a UI de De-Para (Fase B/PR #132).
**Próximo passo:** Obter aprovação do usuário para comitar/enviar as alterações e continuar com a UI de tratativa do de-para direto conta→linha da DRE.

### 2026-07-14 (sessão 22)
**O que foi feito:** Validação local do **modelo definitivo da DRE** (mergeado na sessão 21, PR #135 — `DeParaDreLinha`, motor de vínculo direto conta→linha, mapa de âncoras por demonstrativo). Ambiente local (SQLite clonado de produção em sessões anteriores) reseedado com Fase A/A′; template DRE do Leal recriado no modelo novo (34 linhas, **0 folhas com agrupamento**, 11 vínculos `DeParaDreLinha` conta→linha). **Semeado um balancete de DEMONSTRAÇÃO** (aditivo, autorizado pelo usuário, com os valores mostrados antes de gravar) para provar o modelo de ponta a ponta: jan/2026, unidade **101 (Loja Centro)** do Leal, 12 lançamentos — incluindo a folha "Receita Venda à Vista" com **2 contas** (800 + 450 = 1.250) e uma conta de **rateio 70/30** (1.000 → Despesas Administrativas 700 / Despesas Comerciais 300). A cadeia de margens conferida **na tela** e pelo motor: **RECEITA BRUTA 3.250 → VENDA LÍQUIDA 3.000 → RECEITA LÍQUIDA 2.600 → MARGEM BRUTA 1.000 → MARGEM DE CONTRIBUIÇÃO I 800 → RESULTADO −700** — totalizadores calculando sozinhos a partir das folhas (soma_filhos + fórmula), drill-down por unidade (LOJA CENTRO + CONSOLIDADO) e rateio funcionando. **Bug de UI pré-existente encontrado e corrigido durante a validação** (não relacionado ao backend/motor): a tela **DRE Referencial** (`Demonstrativo.jsx`) era a única das 22 telas de conteúdo que não usava `<div className="page">` — usava um `<div>` com padding solto; como `.main-area` tem `overflow:hidden` e não há wrapper de rolagem, o conteúdo alto (1851px, viewport 694) era clipado sem scrollbar e as últimas linhas (até RESULTADO) ficavam inalcançáveis. Só apareceu agora que a DRE tem 34 linhas de dados. Corrigido trocando o root para `<div className="page">` (padrão das outras 21 telas); verificado ao vivo que passa a rolar até o RESULTADO. **Branch `fix/dre-referencial-scroll`, PR #137 aberta — NÃO mergeada** (aguardando aprovação do usuário).
**Decisões tomadas:** Sem release/deploy — a etapa da DRE ainda **não está completa** (falta a UI de tratativa do de-para direto e a importação real). O SQLite local com o balancete de demonstração fica **preservado** (não apagar) para o usuário retomar os testes depois. O balancete foi apresentado ao usuário item por item antes de gravar, com a cadeia esperada, e só gravado após confirmação explícita. Reforçada a conduta: cópia de produção sempre read-only; nada destrutivo no local sem OK.
**Próximo passo (etapa DRE, antes de qualquer release):** (a) **UI de tratativa** do de-para direto conta→linha — evoluir a Fase B / PR #132 (Preparo DE-PARA, ainda aberta) para o modelo DRE (`DeParaDreLinha`); (b) **importação real** (Fase D) respeitando a ordem DE-PARA-antes-dos-valores. O release da etapa DRE só acontece quando (a) e (b) estiverem prontos. Pendências de PR: **#132** (Fase B, aberta), **#137** (fix de scroll da DRE, aberta).

### 2026-07-13 (sessão 21)
**O que foi feito:** **Modelo Definitivo do Projeto Referencial** — reescrita da fundação após decisões estruturais, em três frentes ao longo da sessão. (1) **Documento-mestre** (`documentos/PROJETO_REFERENCIAL.md`, PRs #133 fechada por sobreposição / **#134 mergeada**): reescritas as seções de conceitos e fluxo com o modelo definitivo — cada demonstrativo tem TEMPLATE PRÓPRIO referencial (compartilhado por segmento, permite benchmark); a **DRE não usa agrupamento** (a linha-folha aponta DIRETO para N contas nativas do cliente, de-para conta→linha); o **FC continua com agrupamento** (74 de produção intactos, `FcSlugDepara` movimento→agrupamento, modelo Rio das Pedras que já funciona); adicionado o **contraste DRE×FC e o porquê** (no FC a mesma conta é entrada E saída → precisa da camada de classificação de movimento; na DRE a conta tem natureza fixa → de-para direto basta); incorporados Plano Nativo (`ContaClienteRef` como espelho do ERP) e a ordem obrigatória DE-PARA-antes-dos-valores (valor de conta sem vínculo fica retido). (2) **Implementação do vínculo direto da DRE** (branch `feature/referencial-dre-vinculo-direto`, **PR #135 mergeada**): novo modelo **`DeParaDreLinha`** (`conta_cliente_id` + `template_linha_id` + `percentual`/rateio + `status` + `confianca` + `origem_vinculo` + `vigente_a_partir`, tabela nova criada por `create_all` no boot, sem ALTER); motor `ref_demonstrativos._calcular_template` — na Fase 1 (folhas), folha **com** `agrupamento_slug` segue no caminho do agrupamento (FC/legado, intacto) e folha **sem** `agrupamento_slug` passa a somar os `LancamentoRef` das contas nativas vinculadas via o novo `_get_valores_dre_por_linha` (por unidade, versionamento por data, rateio por percentual, sem sinal por natureza — valor bruto, subtração fica nos totalizadores); `seed_dre_leal.py` ajustado (folhas sem agrupamento + Plano Nativo mínimo do Leal + 11 vínculos conta→linha, com "Receita Venda à Vista" mapeada a 2 contas para exercitar a soma); 3 testes novos (`tests/test_ref_dre_vinculo_direto.py`: folha com 2 contas soma 1250, totalizador `{linha:...}` sobre folhas diretas, rateio 70/30). (3) **Mapa de âncoras por demonstrativo** registrado no doc: DRE→linha do template (`DeParaDreLinha`); FC→agrupamento (`FcSlugDepara`); **Balancete Gerencial→`ContaReferencial`** (`DeParaRef`); Orçamento→a definir — cada demonstrativo com de-para próprio, sem redundância.
**Decisões tomadas:** `ContaReferencial`/`DeParaRef` com **papel definido** — plano-mestre do **Balancete Gerencial** (consumido em `ref_lancamentos.py`), **mantidos e NÃO aposentados**; o Balancete entra como fase futura reaproveitando essa fundação. Os 24 agrupamentos de DRE da Fase A foram **descartados** (DRE não usa agrupamento); os 74 de FC **intactos**. Opção de vínculo aprovada: **tabela nova** `DeParaDreLinha` (não estender `DeParaRef`) — o `DeParaRef` aponta para `ContaReferencial` (Balancete), alvo semanticamente diferente da linha de template (DRE); reusar exigiria FK polimórfico. Diagnóstico do reaproveitamento da A′ entregue antes de codar (≈80% de pé: template, hierarquia, `soma_filhos`, totalizadores, motor de fórmula, editor — muda só a resolução do valor da folha da DRE). PR #135 mergeada com aprovação explícita do usuário (verificado: campos do modelo, motor somando direto, rateio, FC intacto, 3 testes novos). **Contexto de ambiente:** durante a sessão o SQLite local foi resetado e depois **clonado do Postgres de produção** (cópia SOMENTE-LEITURA no PG — sessão `readonly=True`, só SELECT; nunca escreve em produção) para testar com dados reais (11 clientes, 15 usuários, 916 `ContaReferencial`, 990 `fc_lancamentos`); a conexão direta ao Supabase (porta 5432) **funcionou** desta vez (o bloqueio citado em sessões antigas não estava ativo). Lembrete de conduta reforçado: **não rodar `--force`/reset destrutivo, mesmo em local, sem OK do usuário**.
**Próximo passo:** Fase futura — UI de tratativa do de-para direto da DRE (evoluir a Fase B / PR #132, ainda aberta e não mergeada). Suíte em **103 testes** (100 + 3 novos). PRs abertas pendentes: **#132** (Fase B — Preparo DE-PARA).

### 2026-07-12 (sessão 20)
**O que foi feito:** **Fase A′** do Projeto Referencial (branch `feature/referencial-fase-a-linha-template-e-dre-leal`, PR #130 mergeada) — evolução do editor de template para o modelo "planilha" e geração da DRE-espelho real do cliente Leal. `TemplateLinhaRef` ganhou `modo_calculo` (`'agrupamento'|'soma_filhos'|'formula'`, default `'agrupamento'`) e `nivel` (1=A/bloco, 2=C/grupo, 3=D/subgrupo, 4=E/folha, default 4); `tipo` (antigo) passou a ser só estilo visual, documentado como tal. Schemas (`TemplateLinhaCreate/Update/Out`) passaram a expor `tipo`, `modo_calculo`, `nivel` e `agrupamento_slug` — ausentes até então na API (só existiam via seed direto no banco). Motor (`ref_demonstrativos.py`) reescrito para calcular em 3 fases por linha: folhas `'agrupamento'` (sem dependências) → `'soma_filhos'` de baixo para cima por `nivel`/`ordem` (suporta aninhamento arbitrário de blocos/grupos) → `'formula'` na ordenação topológica e detecção de ciclo já existentes, sem alteração de comportamento — com fallback legado (linha sem `modo_calculo` explícito mas com `formula_texto` e sem `agrupamento_slug` continua tratada como totalizador, preservando templates anteriores à Fase A′ após a migração aplicar o default em massa). Editor (`TemplatesRef.jsx`) ganhou seletor de modo de cálculo e nível, campo Fórmula condicional (só em `'formula'`), dropdown de agrupamentos reais com slug condicional (só em `'agrupamento'`, via `GET /api/fluxo-caixa/agrupadores` agora expondo `slug`), indentação visual por nível e nota fixa "o relatório nunca é editado, só o template". Criado `backend/seed_dre_leal.py`: template "DRE Gerencial - Varejo (Leal)" com 34 linhas mapeando a estrutura real da planilha aos 24 agrupamentos da Fase A — cadeia `receita_bruta → venda_liquida → receita_liquida → margem_bruta → margem_contribuicao_i → resultado`, blocos `soma_filhos` e folhas `'agrupamento'` incluindo as 5 aberturas departamentais (por ora soltas, drill-down a refinar na fase de importação); cliente Leal buscado por CNPJ (`25926205000123`, não por nome) — não encontrado no banco local, reportado sem falhar silenciosamente. **Bug real encontrado e corrigido durante a verificação visual**: `TemplateRefOut.segmento_id` exigia `int` obrigatório, mas o modelo permite `None` (template "universal") — qualquer listagem que incluísse um template sem segmento (ex.: o já existente "Fluxo de Caixa") quebrava com 500 (`ResponseValidationError`), derrubando a tela inteira de Templates de Demonstrativos; não era regressão desta branch, só nunca havia sido exercitado por esse endpoint antes.
**Decisões tomadas:** Usuário confirmou o ponto crítico — as fórmulas das margens (Venda Líquida, Receita Líquida, Margem Bruta, Margem de Contribuição I, Resultado) batem exatamente com a planilha original do cliente Leal. Verificação end-to-end feita chamando `_calcular_template` diretamente com um cliente temporário (sem dados de lançamento): toda a cadeia de fórmulas calculou sem erro de ciclo ou fórmula — os `ref_inexistente` nas folhas eram esperados (sem DE-PARA/lançamentos ainda, fora do escopo desta fase). PR #130 mergeada com aprovação explícita do usuário. `documentos/PROJETO_REFERENCIAL.md` atualizado marcando Fases A e A′ como concluídas (PR #128 e #130).
**Próximo passo:** Usuário vai passar o prompt da **Fase B** (preparo DE-PARA de contas) — não iniciar sem esse prompt.

### 2026-07-12 (sessão 19)
**O que foi feito:** Início do **Projeto Motor DE-PARA e Importação Referencial** (branch `feature/referencial-fase-a-agrupamentos`, PR #128 mergeada) — projeto grande, feito em fases validadas uma a uma. Criado o documento-mestre `documentos/PROJETO_REFERENCIAL.md` com o desenho completo (objetivo, conceitos-chave — AGRUPAMENTO, LINHA DE TEMPLATE com `modo_calculo`, DEPARTAMENTO como dimensão paralela —, fluxo e fases A a F) para que sessões futuras entendam o todo sem precisar reconstruir o contexto. Executada a **Fase A** (reset e carga dos agrupamentos-padrão para DRE varejo): nova coluna `Agrupamento.dimensao` (`'contabil'` | `'departamento'`, migração SQLite+Postgres, exposta em `GET /api/fluxo-caixa/agrupadores`); novo `backend/seed_agrupamentos_ref.py` com dois guards (não apaga agrupamentos com vínculos `ContaAgrupamento` em uso sem `--force`; só roda fora do SQLite local com `--force` explícito) que recria 24 agrupamentos (19 contábeis + 5 departamentais, todos `demonstrativos=["dre"]`), substituindo os 62 antigos do modelo de Fluxo de Caixa (`seed_controladoria.py`). **Bug real encontrado durante o teste local**: `seed_controladoria.seed_agrupadores` roda automaticamente em todo startup do backend e usa a presença do agrupamento `"Vendas - Dinheiro"` como marcador de "já populado" — como os novos agrupamentos não têm esse nome, cada reload do uvicorn (inclusive o do próprio dev, disparado por `--reload` ao salvar qualquer arquivo) disparava um reset silencioso de volta aos 62 agrupamentos antigos, apagando os 24 novos recém-criados. Corrigido removendo a chamada `seed_agrupadores(_db)` do startup de `main.py` — `seed_agrupamentos_ref.py` passa a ser o único responsável pelo reset dos agrupamentos, sempre manual, nunca automático.
**Decisões tomadas:** Reset feito como script standalone (não integrado ao startup) por design — a "Fase A" é uma migração de modelo pontual e explícita, não algo que deva rodar sozinho a cada boot (diferente do `seed_controladoria` antigo, cujo comportamento automático foi exatamente a causa do bug acima). Verificação visual feita logando como admin interno (código `001`) na tela **Plano de Contas Referencial** → painel de vínculo de agrupamento → aba **DRE**, confirmando os 24 agrupamentos novos na lista. Screenshot mostrado diretamente na conversa (não foi possível anexar o PNG binário à PR — o ambiente de preview usado não expõe um caminho de arquivo local para o screenshot, só renderização inline). PR #128 mergeada com aprovação explícita do usuário após ele conferir o resumo da Fase A (documento-mestre, coluna `dimensao`, 24 agrupamentos, guards, bug do restart corrigido, testes verdes).
**Próximo passo:** Usuário vai passar o prompt da **Fase A′** (editor de template evoluído com `modo_calculo` + nível, e template DRE-espelho do cliente Leal com fórmulas reais) — não iniciar sem esse prompt.

### 2026-07-11 (sessão 18)
**O que foi feito:** Criado o padrão visual único de gráficos do sistema, extraído do `PainelDetalheAgrupamento.jsx` (referência aprovada pelo usuário): `frontend/src/components/chartTheme.js` (paleta violeta, tooltip customizado, grid/eixos, formatadores) e `Graficos.jsx` (único arquivo do projeto que importa `recharts` — wrappers `GraficoBarras`/`GraficoLinha`/`GraficoArea`/`GraficoRosca`/`GraficoProgresso`/`GraficoComposto`, todos com prop `dark` para as dashboards do módulo Projetos e escape hatches como `tooltipContent`/`legendaContent`/`centroContent` para lógica bespoke). Migrados **todos** os gráficos recharts do sistema para os wrappers: `PainelDetalheAgrupamento.jsx` (visual idêntico — é a origem do padrão), `PainelDetalheOrcamento.jsx`, `Dashboard.jsx`, `DashboardFases.jsx`, `DashboardSubtarefas.jsx`, `DreDashboard2.jsx` (ComposedChart). `DashboardCliente.jsx` migrado de `plotly.js-dist-min` para os wrappers; `plotly.js`/`plotly.js-dist-min`/`react-plotly.js` removidos do projeto (~3MB a menos no bundle). Quitada a dívida visual do PR #122: `.btn-acao-tabela` de `Clientes.jsx` (hover em `--border-color`, variável inexistente) trocado por `IconButton` de `ui.jsx`; toolbar lateral de `Orcamento.jsx` **e** `FluxoCaixa.jsx` (mesmo bloco duplicado nas duas telas, achado ao investigar) com dezenas de cores hex + `!important` migrada para variáveis CSS novas (`--toolbar-*`) em `index.css`. **Dois bugs reais encontrados e corrigidos durante a validação visual no navegador** (backend+frontend locais, SQLite de dev): (a) `BotaoExportarPDF` (modo `iconOnly`) ignorava a prop `className` do caller — o ícone de exportar PDF da toolbar nunca teve a cor vermelha aplicada em nenhuma das duas telas, sempre caía no estilo cinza padrão do componente; corrigido para aceitar e usar `className`. (b) `GraficoBarras` não desenhava nenhuma barra em vários cenários reais (ex.: comparativo Atual/Anterior do Fluxo de Caixa com múltiplas contas) — recharts calculava `x=NaN` para todo `Bar`. Bissecção empírica (incluindo checkout temporário da `main` para confirmar que o bug NÃO existia lá, portanto era regressão desta branch) identificou dois problemas na montagem condicional dos eixos por layout: eixos agrupados em fragment por ramo do ternário (recharts às vezes não reconhecia como filhos diretos) e `width={undefined}` passado explicitamente ao `YAxis` (em vez de omitido), que quebrava o cálculo interno de offset do eixo. Corrigido renderizando exatamente um `XAxis`/`YAxis` por ternário, nunca com prop de valor `undefined` explícito. Documentação: seção 11 "Gráficos" no `DESIGN_SYSTEM.md` (paleta oficial, quando usar cada wrapper, regra de proibido `recharts` direto), espelhada no CONTRATO DE SESSÃO (`CLAUDE.md`+`AGENTS.md`), registro no `ROADMAP_2.md`.
**Decisões tomadas:** DashboardCliente/Dashboard/DashboardFases/DashboardSubtarefas (dashboards do módulo Projetos) mantiveram o fundo escuro próprio por decisão explícita do usuário ("esse Dash é ref. ao Módulo de projetos") — os wrappers ganharam a prop `dark` para adaptar tooltip/grid/eixo sem mudar a gramática do gráfico nem forçar o padrão claro/violeta ali. PR #123 mergeado com aprovação visual do usuário via validação interativa no navegador (não foi possível automatizar a captura/anexo de screenshots PNG nesta sessão — Chrome MCP dedicado sem sessão autenticada válida e sem credenciais disponíveis para logar; validação feita ao vivo e descrita no PR). Release `v2.6.2u` via `/release` após `/conferencia-pre-release` retornar GO (92 testes, build limpo, nenhuma migração de schema necessária).
**Próximo passo:** Confirmar pós-deploy que `/api/version` retorna `2.6.2u` e lembrar o usuário do **Ctrl+Shift+R** no Electron.
