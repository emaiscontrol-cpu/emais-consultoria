...
export default function Arquivos() {
  ...
  const handleUpload = async (e) => {
    ...
    for (const f of files) {
      try {
        await arquivosAPI.upload(clienteAtivo.id, f)
        ok++
      } catch {
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
              {loadingArquivos ? (
                ...
                <div style={{ textAlign: 'center', padding: 60, color: 'var(--text-muted)' }}>
                  ...
                  <div style={{ fontSize: 14 }}>Nenhum arquivo enviado ainda.</div>
                  <div style={{ fontSize: 12, marginTop: 4 }}>Clique em "Enviar arquivo" para adicionar documentos.</div>
                </div>
              ) : (
